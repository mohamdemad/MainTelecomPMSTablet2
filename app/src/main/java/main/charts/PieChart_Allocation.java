...
public class PieChart_Allocation extends Fragment  {

	private RelativeLayout mainLayout;
	private PieChart chart;
	private float[] yData = new float[2];
	private String[] xData = { "Allocated", "Not Allocated"};
	...
	public View onCreateView(LayoutInflater inflater,
		...
		View fragment_pie_allocation = inflater.inflate(
		...
		mainLayout = (RelativeLayout) fragment_pie_allocation
				.findViewById(R.id.layout_piechart_invoices);

		 serverUrl = new StringBuilder("http://"+ Login.enteredIP+"/Android/Team/Organization_Allocation.php").toString();

		 Organization_Allocation_Connection Allocation_connection = new Organization_Allocation_Connection(getContext());
		 Allocation_connection.execute(serverUrl);

		return fragment_pie_allocation;
	...
	private void pie_chart() {
		...
		chart.setDescription("Organization Allocation");
	...
	private class Organization_Allocation_Connection extends AsyncTask<String, Void, String> {
		...
		public Organization_Allocation_Connection(Context context) {
		...
		public ArrayList<Float> returnParsedJsonObject(String result) {
			...
			try {
				...
				float allocated = Float.valueOf(resultObject.getString("Allocated"));
				float not_allocated = Float.valueOf(resultObject.getString("Not_Allocated"));

				if (allocated<0){allocated=0;}
				if (not_allocated<0){not_allocated=0;}

				Result.add(Float.valueOf(allocated));
				Result.add(Float.valueOf(not_allocated));
